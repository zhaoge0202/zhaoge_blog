...
语言与类型 → final/static/不可变 → 面向对象 → 枚举 → 异常 → 泛型 → 反射、代理、Lambda、序列化与 SPI
...
- [final、static、不可变对象到底怎么理解？](./java-basis-final-static-immutable.html) — 变量绑定、类成员、初始化顺序和不可变对象设计。
...
- [枚举 enum 为什么比常量类更适合表达固定集合？](./java-basis-enum.html) — 类型安全、字段行为、EnumMap/EnumSet 和枚举单例边界。
...
- [Lambda 和方法引用底层是怎么实现的？](./java-basis-lambda-invokedynamic.html) — 函数式接口、`invokedynamic`、方法引用和捕获变量。
- [Java 序列化为什么不推荐直接用于长期存储和接口传输？](./java-basis-serialization.html) — `serialVersionUID`、字段规则、兼容性和反序列化安全。
