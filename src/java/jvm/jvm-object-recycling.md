...
可达性分析不是只在面试里背概念。线上看堆 dump 时，真正要问的是：“这个大对象为什么还被某条根路径牵着？”

比如一个服务老年代一直涨，dump 里看到很多 `OrderDTO`，不要只停在“对象很多”。要继续追持有链：
...
这条路径说明对象不是“GC 不工作”，而是业务线程的 `ThreadLocal` 还持有它。修复方向就不是换收集器，而是请求结束后 `remove()`，或者避免把大对象塞进线程上下文。

常用证据链是：

```bash
# 先粗看类实例数量，必要时会触发一次 GC，生产上要评估影响
jcmd <pid> GC.class_histogram

# 再保留堆现场，给 MAT/VisualVM 分析 GC Roots 路径
jmap -dump:format=b,file=/data/dumps/app.hprof <pid>
```

如果机器已经非常卡，不要一上来就做大 dump。先确认磁盘空间、进程可承受的停顿，再决定是否在备用节点或低峰执行。
...
- 强引用问题最常见：静态集合、缓存、线程上下文、监听器列表忘记移除。
- 软引用不是可靠缓存策略：回收时机受 JVM 策略和内存压力影响，命中率不可控。
- 弱引用适合“对象还活着就附带一些信息”，例如弱 key 映射；不适合保存核心业务数据。
- 虚引用通常和引用队列配合，用来感知对象即将被回收，更多出现在堆外资源清理框架里。

判断引用类型时别只看类名，要看有没有其他强引用路径。一个对象同时被强引用和弱引用指向时，弱引用不会让它更容易被回收。
...
如果自定义 ClassLoader 数量持续上升，且旧加载器无法回收，常见根因是：

- 静态字段缓存了应用类或 ClassLoader。
- 线程上下文 ClassLoader 没恢复。
- 线程池里的线程跨版本复用，继续持有旧类。
- 注册到全局单例的 JDBC Driver、MBean、监听器没有注销。

这类问题会表现为元空间增长，严重时触发元空间相关 GC，甚至 OOM。排查时要把“类能不能卸载”与“加载它的 ClassLoader 能不能断开引用”放在一起看。

## 容易踩的坑

1. “不可达”不等于“立刻释放内存”，真正释放要等具体 GC 周期执行到回收阶段。
2. `System.gc()` 只是建议 JVM 做 GC，不是强制释放某个对象。
3. 软引用不是现代缓存的首选方案，容量、过期、淘汰和统计都不可控。
4. heap dump 只说明某一刻的对象关系，分析时要结合时间线和 GC 日志。
5. 类卸载要看 ClassLoader 引用链，不能只看实例对象是否没了。
...
- 类卸载必须连同实例、Class 对象和 ClassLoader 引用一起解除，排查时要追 GC Roots 路径。
