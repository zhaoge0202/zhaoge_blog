...
还要注意大小写和连字符并不可靠：资料、配置和抓包里可能写成 `Keep-Alive`、`keepalive`、`SO_KEEPALIVE`。真正判断它属于哪一层，要看它是在 HTTP Header、Web 服务器连接复用配置，还是 socket/TCP 内核参数里出现。
...
`Connection` 和 `Keep-Alive` 都是 hop-by-hop 头部，也就是只作用于当前这一段连接。客户端到网关、网关到后端应用，可能是两条不同的 TCP 连接，Keep-Alive 策略也可以完全不同。排查经过代理的请求时，不要把客户端看到的 Header 直接等同于后端连接状态。

## 长连接超时错配会造成什么现象？

线上很常见的一类问题是：客户端连接池认为连接还能复用，服务端或中间代理已经把空闲连接关了。

```mermaid
sequenceDiagram
 participant C as 客户端连接池
 participant P as 网关/代理
...
- 客户端连接池 idle timeout 是否小于服务端 keepalive timeout。
- 代理前后两段连接的超时是否一致，比如 SLB/Nginx/网关/后端各自的 idle timeout。
- 服务端是否通过 `Connection: close`、GOAWAY 或 FIN 提前表达不再接收新请求。
- 高 QPS 下 `keepalive_requests` 是否太小，导致频繁主动关闭和大量 `TIME_WAIT`。
...
如果业务需要比系统默认更快地发现死连接，通常要优先看语言和网络库是否支持逐 socket 配置，例如 Linux 上的 `TCP_KEEPIDLE`、`TCP_KEEPINTVL`、`TCP_KEEPCNT`。不要为了一个应用直接把全局 sysctl 改得很激进，否则同机其他服务也会受影响。
...
对 HTTP/2 和 HTTP/3 也要分开看：

- HTTP/2 不使用 HTTP/1.x 的 `Connection: Keep-Alive`，但底层 TCP 连接仍可能开启 TCP Keepalive，协议层还可以用 PING 帧做应用层存活探测。
- HTTP/3 跑在 QUIC 上，不受 TCP Keepalive 直接影响；QUIC 自己有 idle timeout 和 PING 之类的连接管理手段。
...
不过 `TIME_WAIT` 多不一定就是坏事。它本来就是 TCP 主动关闭方保护旧连接报文、避免四元组复用混淆的机制。真正需要处理的是“短连接过多导致端口、CPU、连接表或日志异常”，而不是看到 `TIME_WAIT` 数字大就立刻改内核参数。
...
- 经过代理时 Keep-Alive 是分段生效的，超时错配会引发 502、RST、重试和大量 `TIME_WAIT`。
