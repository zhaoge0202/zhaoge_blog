...
## “没有 accept”和“处理慢”怎么分清？

这两个问题经常一起出现，但定位方向不同。

| 场景                     | 连接处在哪                              | 客户端表现                                   | 服务端重点看什么                              |
| ------------------------ | --------------------------------------- | -------------------------------------------- | --------------------------------------------- |
| 没有及时 `accept()`      | 已完成握手，卡在全连接队列              | `connect()` 可能成功，请求首包没人处理       | `ss -ltn` 监听行的 `Recv-Q/Send-Q`            |
| 已 `accept()` 但没及时读 | 已进入应用 fd，数据堆在 socket 接收缓冲 | 请求发出后读超时，服务端可能有已建立连接堆积 | 非 `LISTEN` 连接的 `Recv-Q`、线程栈、事件循环 |
| 已读但业务处理慢         | 进入协议解析或业务线程                  | 响应慢、超时、连接数上升                     | 线程池、慢 SQL、下游 RPC、GC                  |

所以 `connect()` 成功只能说明建连视角没立即失败，不能证明服务端应用已经开始处理这个请求。

## 客户端抓包通常会看到什么？

从客户端抓包看，常见现象有三类：

1. 队列没满但应用不 accept：能看到 `SYN -> SYN+ACK -> ACK`，`connect()` 返回成功；客户端随后发请求数据，但长时间收不到应用层响应，可能只看到 TCP 层 ACK 或后续读超时。
2. 队列满且服务端丢第三次握手 ACK：客户端可能以为自己已进入 `ESTABLISHED`，继续发送数据；服务端可能重传 `SYN+ACK`，客户端继续回 ACK，最终表现为连接慢、重传或超时。
3. 队列满且快速失败：服务端回 RST，客户端更快看到 `Connection reset by peer` 或连接失败。

抓包的价值是把“建连阶段失败”“首包没人处理”“应用响应慢”拆开，而不是只看一个超时异常。
...
这里看的是 `LISTEN` 行。如果看的是某条 `ESTABLISHED` 连接，`Recv-Q` 就不再表示等待 accept 的连接数，而是已收到但还没被应用读取的字节数。
...
nstat -az | egrep "TcpExtListenOverflows|TcpExtListenDrops"
```
...
如果是 Netty，先区分 boss group 和 worker group：boss 负责接收新连接，worker 负责已连接 socket 的读写事件。boss 卡住更像 accept 不及时；worker 或业务线程卡住，更像连接已进入应用但请求处理慢。如果是 Tomcat，要同时看 Acceptor、Poller、工作线程、`acceptCount`、最大连接数和请求线程池，避免把所有超时都归因成“没 accept”。
...
- 排查要分清没 accept、已 accept 但没读、已读但业务慢，再结合 `ss -ltn`、`nstat`、`netstat -s`、线程栈、GC、fd 和 backlog/somaxconn。
