...
还有一个常见混淆：RSA 可以用于“密钥交换”，也可以用于“证书签名”。TLS 1.3 移除的是传统 RSA 密钥交换，不代表 RSA 证书或 RSA 签名算法完全不能出现。面试里如果只说“TLS 1.3 没有 RSA”，容易被追问出漏洞。
...
1. 证书链能否一路验证到本地信任的根证书。
2. 证书是否在有效期内，是否被吊销。
3. 访问域名是否匹配证书的 SAN 或通配符规则。
4. 证书用途、签名算法、密钥长度是否满足当前客户端策略。

线上遇到 `certificate verify failed`，不要只说“证书错了”，要先区分是链不完整、域名不匹配、过期、客户端根证书过旧，还是中间代理替换了证书。
...
## TLS 1.2 和 TLS 1.3 的回答边界是什么？

上面 RSA/ECDHE 的对比主要站在 TLS 1.2 语境下讲。TLS 1.3 做了更激进的简化：

- 去掉传统 RSA 密钥交换、静态 DH、一些旧的 CBC/RC4 类套件。
- 握手默认走前向安全的 Diffie-Hellman/ECDHE 路线。
- 典型首次握手只需要 1-RTT 就能完成密钥协商和认证。
- 会话恢复可以支持 0-RTT，但 0-RTT 数据要面对重放风险。

所以更准确的表述是：现代 HTTPS 的主流路线是“证书认证身份 + ECDHE 类密钥协商 + AEAD 对称加密”。RSA 仍可能作为证书签名算法出现，但不再承担 TLS 1.3 的密钥交换职责。
...
| 信息                 | 看什么                                              |
| -------------------- | --------------------------------------------------- |
| `Protocol`           | 实际协商到 TLS 1.2 还是 TLS 1.3                     |
| `Cipher`             | 是否是 ECDHE、AES-GCM、ChaCha20-Poly1305 等现代套件 |
| `Certificate chain`  | 服务端是否把中间证书链发完整                        |
| `Verify return code` | 证书验证是否成功                                    |
| `ALPN protocol`      | 是否协商到 `h2` 或回退到 `http/1.1`                 |

如果要验证域名，一定带 `-servername`。这是 SNI，表示客户端想访问哪个域名。很多网关一个 IP 绑定多张证书，不带 SNI 可能拿到默认证书，误判成证书错误。
...
- TLS 1.3 已移除传统 RSA 密钥交换，但 RSA 仍可能作为证书签名算法出现，回答时要区分签名和密钥交换。
- 排查 HTTPS 要看证书链、SNI、TLS 版本、密码套件和 ALPN，不能只看端口 443 是否通。
