...
这也解释了虚拟线程的边界：它适合大量“等 IO”的任务，不会让 CPU 密集计算突破 CPU 核数限制。还要带版本边界看 pinning：JDK 21-23 中，虚拟线程在 `synchronized` 临界区里执行阻塞操作时可能占住承载它的平台线程；JDK 24 起，`synchronized` 相关 pinning 已被大幅消除。即便如此，native 调用、类加载初始化等少数无法挂起的阻塞点仍可能占住承载线程，排查时不能只看虚拟线程数量。
...
- “虚拟线程里不能用 `synchronized`”：这个说法要看 JDK 版本。JDK 24+ 已改善 `synchronized` pinning，是否需要换成 `ReentrantLock` 应该回到锁语义、可中断、公平性和观测证据，而不是机械替换。
