...
虽然大多数业务里的“事务失效”都先看代理和回滚规则，但也不能把数据库层完全排除。

排查前先确认 3 件事：

1. 当前数据库或存储引擎是否支持事务。
2. 这次操作走的数据源是否被对应事务管理器接管。
3. 项目里是否存在多事务管理器、多数据源路由或非关系型资源混用。

如果底层资源本来就没有事务语义，或者事务管理器根本没有管到这条连接，后面的代理、传播行为、回滚规则都没有落点。
...
反过来也要看 `noRollbackFor`。有些业务异常可能被显式配置成不回滚，或者父类异常规则和子类异常规则互相影响。排查时不能只看“抛了什么异常”，还要看事务注解上完整的回滚规则。
...
| 传播行为        | 容易误判的点                                                     |
| --------------- | ---------------------------------------------------------------- |
| `SUPPORTS`      | 有事务就加入，没有事务就非事务执行，所以“有注解”不代表一定开事务 |
| `NOT_SUPPORTED` | 会挂起当前事务后非事务执行，本来就是主动绕开事务                 |
| `NEVER`         | 当前存在事务时直接报错，不是“不开新事务但继续跑”                 |
| `MANDATORY`     | 必须已有事务，没有事务会报错                                     |

所以排查传播行为时，要先问一句：这个配置到底是在加入、开启、挂起，还是强制要求已有事务？
...
`@Transactional` 里除了传播行为和回滚规则，还有几个属性也经常被误用。

| 属性        | 容易误解                | 更稳的理解                                                    |
| ----------- | ----------------------- | ------------------------------------------------------------- |
| `readOnly`  | 写了就绝对不能写        | 更多是给事务管理器、数据库或 ORM 的优化提示，不能替代权限控制 |
| `isolation` | Spring 自己实现隔离级别 | 最终还是落到数据库连接和数据库引擎能力上                      |
| `timeout`   | 能中断所有慢 SQL        | 取决于事务管理器、JDBC 驱动和数据库执行行为                   |

这些属性本质上都属于事务定义。Spring 负责把定义传给具体事务管理器，最终能不能执行到位，要看事务管理器、驱动和数据库资源本身。

所以如果有人说“我把 readOnly=true 打开了，为什么还是写进去了”，答案通常是：它不是业务只读保护。要禁止写入，应该从权限、路由、SQL 审计或数据库账号层面兜住。

隔离级别也一样。Spring 只是把隔离要求传给底层资源，具体有没有效果、行为是什么，要看数据库和驱动支持。比如 MySQL、PostgreSQL、Oracle 对默认隔离级别和细节处理都不完全一样。

## 事务和外部调用不要绑在一起

事务方法里夹 RPC、HTTP 调用、消息发送、文件处理，是线上很常见的隐患。

这不一定是典型的 `@Transactional` 失效，更像是容易被误判成事务问题的工程边界风险。

原因很直接：

- 数据库连接会被事务长时间占用。
- 外部调用慢会拉长锁持有时间。
- 外部调用成功但事务回滚，会产生跨系统不一致。
- 事务提交成功但消息发送失败，也会产生补偿问题。

更稳的设计是缩短数据库事务，把外部副作用放到事务外，或者用本地消息表、事务同步回调、可靠消息这类机制做一致性衔接。
...
4. 底层资源、数据源和事务管理器是否支持并接管事务？
5. 异常类型有没有触发回滚？
6. 异常是不是被自己吞了？
7. `rollbackFor` / `noRollbackFor` 是否符合预期？
8. 传播行为是不是和预期一致？
9. 事务管理器 / 上下文是不是配错了？
10. `readOnly`、`isolation`、`timeout` 这些属性是不是被误解了？
11. 事务里有没有夹外部慢调用或副作用？
```
...
3. 不在同类内部自调用事务方法
4. 需要回滚 checked exception 时显式写 `rollbackFor`，同时谨慎使用 `noRollbackFor`
5. 审计、补偿、消息发送这类“必须独立提交”的逻辑，先明确边界再决定是否用 `REQUIRES_NEW`
6. 事务里只放必要的数据库操作，避免夹 RPC、HTTP 和长耗时逻辑
...
### “readOnly=true 就一定不会写库”

不稳。它更像只读事务语义和优化提示，不是业务安全策略。真正禁止写库要靠数据源路由、账号权限、SQL 审计或代码约束。

## 小结

- `@Transactional` 本质上还是 AOP 代理，没经过代理的调用不会有事务语义。
- 自调用、初始化阶段调用、非 Spring 管理对象，是最常见的“根本没进事务代理”场景。
- 默认只对 `RuntimeException` 和 `Error` 回滚，checked exception、`rollbackFor`、`noRollbackFor` 要一起看。
- `REQUIRED`、`REQUIRES_NEW`、`NESTED` 等传播行为对应不同边界，理解错也会被误判成事务失效。
- `readOnly`、`isolation`、`timeout` 都要看底层事务管理器和数据库支持，不能当成万能开关。
