...
import {
  ...
  readContentManifest,
} from "../scripts/lib/content-inventory.mjs";
...
test("published markdown inventory matches the committed manifest", () => {
  const actual = collectPublishedMarkdown(repoRoot);
  const expected = readContentManifest(repoRoot);

  const actualSet = new Set(actual);
  const expectedSet = new Set(expected);

  const added = actual.filter((file) => !expectedSet.has(file));
  const removed = expected.filter((file) => !actualSet.has(file));

  const hint = `内容清单已过期，请运行 \`npm run manifest:update\` 刷新 ${MANIFEST_RELATIVE_PATH}`;
